...
let dimensions = { xMin: -6, xMax: 6, yMin: -6, yMax: 6 };
...
function plotPoint(vals, n, nP, color) {
  ...
  const dir =
    ctx._pointDirCache[n] || common.pickLabelDirection(ctx, p[0], p[1]);
  ...
  function step(timestamp) {
    ...
    if (timestamp !== prev) {
      ...
      if (!finished["tangent"]) {
        ...
        ctx.moveTo(...pointToCtx(vals, startPoint.x, startPoint.y));
        const dest = {
          x: startPoint.x + mult * (negR.x - startPoint.x),
        ...
        ctx.lineTo(
          ...pointToCtx(vals, negR.x, negR.y - cache.negLeg * mult, true),
        );
        ctx.stroke();
        ctx.setLineDash([]);
...
export { drawCurve, plotNPs, runAddDemo, runAssocDemo };
